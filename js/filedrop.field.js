  ...
  FileDropbox.prototype = {
    ...
    beforeEach: function (file) {
      if (
        ...
        this.uploads.length >= this.options.maxfiles
      ) {
    ...
    uploadStarted: function (i, file, n, xhr) {
      ...
      this.uploads.some(function (e) {
        if (e.data("file") == file) {
          ...
          setTimeout(function () {
            e.find(".progress-bar").removeClass("progress-bar-striped active");
          }, 50);
          return true;
        }
      });
      ...
      this.uploads.some(function (e) {
        if (e.data("file") == file) {
          if (!json || !json.id)
            ...
            // HTTP 500
            return e.remove();
          e.find('[name="' + that.options.name + '"]').val(
            "" + json.id + "," + file.name
          );
          e.data("fileId", json.id);
          e.find(".progress-bar").width("100%").attr({ "aria-valuenow": 100 });
          e.find(".trash").show();
    ...
    addNode: function (file) {
      ...
      filenode
        ...
        .append(
          $('<span class="filename ltr"></div>')
            .append(
              $('<span class="filesize"></span>').text(
                this.fileSize(parseInt(file.size))
              )
            )
            .append(
              $('<div class="pull-right cancel"></div>')
                .append(
                  $('<i class="icon-remove"></i>').attr("title", __("Cancel"))
                )
      ...
      if (file.id) filenode.data("fileId", file.id);
      if (file.download_url) {
        filenode
          .find(".filename")
          .prepend(
            $('<a class="no-pjax" target="_blank"></a>')
              .text(file.name)
              .attr("href", file.download_url)
          );
      } else filenode.find(".filename").prepend($("<span>").text(file.name));
...
(function ($) {
  ...
  $.fn.filedrop = function (options) {
    ...
    function getBuilder(filename, filedata, mime, boundary) {
      ...
      builder.push(crlf + dashdash + boundary + dashdash + crlf);
    ...
    function upload() {
      ...
      if (opts.allowedfiletypes.push && opts.allowedfiletypes.length) {
        for (var fileIndex = files.length; fileIndex--; ) {
          if (
            ...
            $.inArray(files[fileIndex].type, opts.allowedfiletypes) < 0
          ) {
      ...
      if (
        opts.allowedfileextensions.push &&
        opts.allowedfileextensions.length
      ) {
        for (var fileIndex = files.length; fileIndex--; ) {
          ...
          for (i = 0; i < opts.allowedfileextensions.length; i++) {
            if (
              files[fileIndex].name
                .substr(
                  files[fileIndex].name.length -
                    opts.allowedfileextensions[i].length
                )
                .toLowerCase() == opts.allowedfileextensions[i].toLowerCase()
            ) {

...
(function (R$) {
  ...
  R$.add(
    "class",
    "inspector.parser",
    $R.extend(stockInspectorParser.prototype, {
      _getClosestUpNode: function (selector) {
        var $el = this.$el
          .parents(selector, ".redactor-in-" + this.uuid)
          .last();
  ...
  R$.add("plugin", "draft", {
    ...
    start: function () {
      if (!this.opts.draftNamespace) return;
      ...
      if (this.opts.draftId) {
        ...
        this.opts.clipboardUpload = this.opts.imageUpload =
          this.autoCreateUrl + "/attach";
      ...
      if (autosave_url) this.app.api("module.autosave.enable");

      if (this.app.source.getCode()) this.app.broadcast("draft.recovered");
    },
    ...
    _setup: function (draft_id) {
      ...
      if (this.opts.draftDelete) {
        ...
        var trash = (this.deleteButton = this.toolbar.addButton("deletedraft", {
      ...
      if (this.opts.draftSave) {
        var save = (this.saveButton = this.toolbar.addButton("savedraft", {
          title: __("Save Draft"),
    ...
    onautosaveError: function (error) {
      ...
      this.statusbar.add(
        "draft",
        '<span style="color:red">{}</span>'.replace("{}", __("save error"))
      );
    ...
    deleteDraft: function () {
      ...
      $.ajax("ajax.php/draft/" + this.opts.draftId, {
        ...
        success: function () {
          ...
          self.opts.autosave = self.autoCreateUrl;
          self.opts.clipboardUpload = self.opts.imageUpload =
            self.autoCreateUrl + "/attach";
  ...
  R$.add(
    "service",
    "toolbar",
    $R.extend(stockToolbar.prototype, {
      addButtonAuto: function (name, btnObj) {
        var pos = this.opts.buttons.indexOf(name);

        if (pos === -1) return this.addButton(name, btnObj);
        if (pos === 0) return this.addButtonFirst(name, btnObj);
  ...
  R$.add("plugin", "autolock", {
    ...
    start: function () {
      ...
      if (code.length) this.lock = root.closest("[data-lock-object-id]");
    },
    onchanged: function (e) {
      if (this.lock) this.lock.exclusive("acquire");
    },
  });
  ...
  R$.add("plugin", "signature", {
    ...
    start: function () {
      ...
      if (signatureField) {
        this.$signatureBox = $R
          .dom('<div class="selected-signature"></div>')
          .append(inner);
        this.app.editor
          .getElement()
          .parent()
          .find(".redactor-statusbar")
          .before(this.$signatureBox);
        if ($el.data("signature")) inner.html($el.data("signature"));
        else this.$signatureBox.hide();
        $R.dom("input[name=" + signatureField + "]", $form).on(
          "change",
    ...
    updateSignature: function (e) {
      var $el = $(this.app.rootElement),
        ...
        $form = $el.closest("form"),
        selected = $(
          ":input:checked[name=" + signatureField + "]",
          $form
        ).val(),
        type = $R.dom(e.target).val(),
        dept = $R
          .dom(":input[name=" + $el.data("dept-field") + "]", $form)
          .val(),
      ...
      if (selected == "dept" && $el.data("dept-id"))
        ...
        else return inner.empty().parent().hide();
      } else if (selected == "theirs" && $el.data("poster-id")) {
        url += "agent/" + $el.data("poster-id");
      } else if (type == "none") return inner.empty().parent().hide();
      else url += selected;
...
$(function () {
  var captureImageSizes = function (html) {
    ...
    redact = ($.fn.redact = function (el, options) {
      ...
      var options = $.extend(
        {
          ...
          callbacks: {
            start: function () {
              ...
              if ($element.data("width")) $editor.width($element.data("width"));
              $editor.addClass("no-pjax");
              $editor.attr("spellcheck", "true");
              var lang = $element.closest("[lang]").attr("lang");
              if (lang) $editor.attr("lang", lang);
              // Fixup class for
              $element
                .parent()
                .closest(":not(.redactor-box)")
                .addClass("-redactor-container");
            },
          },
          linkSize: 100000,
      ...
      if (true || "scp") {
        // XXX: Add this to SCP only
        options["plugins"].push("contexttypeahead");
      }
      if (el.hasClass("fullscreen")) options["plugins"].push("fullscreen");
      if (el.data("translateTag")) options["plugins"].push("translatable");
      if ($("#thread-items[data-thread-id]").length)
        options["imageManagerJson"] +=
          "?threadId=" + $("#thread-items").data("threadId");
      getConfig().then(function (c) {
        if (
          c.lang &&
          c.lang.toLowerCase() != "en_us" &&
          Redactor.lang[c.short_lang]
        )
          options["lang"] = c.short_lang;
        if (c.has_rtl) options["plugins"].push("textdirection");
        if (el.find("rtl").length) options["direction"] = "rtl";
        if (c.editor_spacing == "single") options.breakline = true;
...
$(document).ajaxError(function (event, request, settings) {
  if (
    settings.url.indexOf("ajax.php/draft") != -1 &&

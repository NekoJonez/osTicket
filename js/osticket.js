...
$(document).ready(function () {
  ...
  var showNonLocalImage = function (div) {
    ...
    if ($div.attr("width")) $img.width($div.attr("width"));
    if ($div.attr("height")) $img.height($div.attr("height"));
  };
  ...
  $(".thread-entry").each(function (i, te) {
    ...
    extra.append(
      $("<a>")
        ...
        .prepend($("<i>").addClass("icon-picture"))
    );
    imgs.each(function (i, img) {
      var $img = $(img);
      ...
      if (!$img.width()) $img.width(($img.attr("width") || "80") + "px");
...
showImagesInline = function (urls, thread_id) {
  ...
  $(selector).each(function (i, el) {
    ...
    if (info && !e.data("wrapped")) {
      ...
      var timeout,
        caption = $('<div class="image-hover">').css({ float: e.css("float") });
      e.wrap(caption)
        .parent()
        .append(
          $('<div class="caption">').append(
            $(
              '<a href="' +
                info.download_url +
                '" class="button action-button pull-right no-pjax"><i class="icon-download-alt"></i></a>'
            )

...
!(function ($) {
  ...
  var Typeahead = function (element, options) {
    ...
    if (this.options.scroll) this.$menu.addClass("scroll");
  ...
  Typeahead.prototype = {
    ...
    select: function (e) {
      ...
      if (typeof this.onselect == "function")
        if (false === this.onselect(val, e)) return;
    ...
    render: function (items) {
      ...
      items = $(items).map(function (i, item) {
        ...
        if (!that.strings) {
          if (item[that.options.render]) item = item[that.options.render];
          else item = item[that.options.property];
        }
    ...
    adjustScroll: function (next) {
      var top = this.$menu.scrollTop(),
        ...
        pos = next.position();
      if (pos.top < 0) this.$menu.scrollTop(top + pos.top - 10);
      else if (next.height() + top + pos.top > bottom)
        this.$menu.scrollTop(
          top + pos.top - this.$menu.height() + next.height() + 10
        );
    },
